...
Table~
\ref{tab:novelty} contrasts the proposed framework with representative prior work across key design dimensions.
...
Algorithm~
\ref{alg:hybrid} summarizes the complete inference-time prediction procedure. The binary meta-classifier (Algorithm~
\ref{alg:binary_meta}) describes the training-time procedure for learning the binary non-flat direction predictor used in the fusion step.
...
\ref{eq:channel_weights}; weights 0.50/0.25/0.15/0.10 for direct/sector/macro/global.
...
Table~
\ref{tab:v7_binary_paired_benchmark} reports the binary non-flat paired benchmark on $n=5,116$ non-flat rows.
...
Table~
\ref{tab:statistical_tests} summarizes all statistical tests. All paired comparisons use 2,000 bootstrap resamples (for F1, MAE, RMSE) or McNemar's test on paired directional predictions (for DirAcc/F1 classification). Raw and adjusted $p$-values are reported.
...
Table~
\ref{tab:wf_metadata} reports walk-forward window metadata. Each window uses expanding training with a test window of approximately 3 months for W1-W3, 2 months for W4, and 1 month for W5. The test windows are non-overlapping consecutive periods.
...
\ref{tab:walk_forward}) & - & - & - & - & 9,062 & 0.5418 \\
...
\ref{tab:wf_metadata}) show that Full Hybrid outperforms baselines in all 5 windows. The standard deviation of F1 across windows is 0.003, indicating stable performance across temporal regimes. Per-window baseline comparisons and statistical tests are available in the supplementary material.
...
Table~
\ref{tab:conformal} reports empirical coverage and average interval width. Under global calibration, coverage meets or exceeds the nominal 95\% target. Wilson score intervals for coverage are reported in the supplementary material.
...
\item \textbf{Ablation study (Table \ref{tab:ablation}):} \texttt{python backend/scripts/run\_ablation.py}
\item \textbf{Three-class paired benchmark (Table \ref{tab:v5_paired_benchmark}):} \texttt{python backend/scripts/run\_paired\_benchmark.py -protocol three-class}
\item \textbf{Binary non-flat benchmark (Table \ref{tab:v7_binary_paired_benchmark}):} \texttt{python backend/scripts/run\_paired\_benchmark.py -protocol binary-non-flat}
\item \textbf{Walk-forward evaluation (Table \ref{tab:walk_forward}):} \texttt{python backend/scripts/run\_walk\_forward.py -windows 5}
\item \textbf{Conformal prediction (Table \ref{tab:conformal}):} \texttt{python backend/scripts/run\_conformal.py -alpha 0.05 -calib-size 220}
\item \textbf{Statistical tests (Table \ref{tab:statistical_tests}):} \texttt{python backend/scripts/run\_stat\_tests.py -bootstrap 2000 -holm}
\item \textbf{Paper compilation:}
\end{itemize}

\textbf{Table-to-script mapping:}
\begin{itemize}
\item Table \ref{tab:data_dist_source}, \ref{tab:data_dist_ticker} to \texttt{build_dataset.py}
\item Table \ref{tab:ablation}, \ref{tab:v5_paired_benchmark}, \ref{tab:v7_binary_paired_benchmark} - \texttt{run\_ablation.py}, \texttt{run\_paired\_benchmark.py}
\item Table \ref{tab:wf_metadata}, \ref{tab:walk_forward} - \texttt{run\_walk\_forward.py}
\item Table \ref{tab:conformal} - \texttt{run\_conformal.py}
\item Table \ref{tab:statistical_tests} - \texttt{run\_stat\_tests.py}
\item Table \ref{tab:scer_ablation} - \texttt{run\_ablation.py}
\item Tables \ref{tab:three_class_bridge}, \ref{tab:three_class_negative_sweep} - \texttt{run_remediation.py}
\item Table \ref{tab:threshold} - \texttt{run_threshold_sweep.py}
\item Figures - \texttt{paper/figures/} (generated by \texttt{run_figures.py})
\end{itemize}
